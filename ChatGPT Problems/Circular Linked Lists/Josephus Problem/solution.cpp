...
int josephus(Node *tail, int stepCount)
{
    ...
    while (tail != tail->next)
    {
        ...
        // It stops 'early' since the actual deletion only occurs on the next node : tail->next
        ...
        /*
            Explanation of : tail->next = toDelete->next
            
            What's happening:
                - 'Removes' the toDelete node from the circular LL
                    - The line is pretty much saying:
                        - Take the pointer of 'tail' and have it point to the next node AFTER where toDelete is currently pointing at
